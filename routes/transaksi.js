...
const detailtransaksi = models.detailtransaksi
...
app.get("/newest", (req,res) => {
    let sql = `SELECT transaksi.*,member.nama as nama_member , detailtransaksi.*, paket.*, outlet.*, (SELECT SUM(total_harga) FROM detailtransaksi INNER JOIN transaksi ON transaksi.id_transaksi = detailtransaksi.id_transaksi ORDER BY transaksi.id_transaksi DESC LIMIT 10) as total FROM transaksi INNER JOIN member ON member.id_member = transaksi.id_member INNER JOIN detailtransaksi ON detailtransaksi.id_transaksi = transaksi.id_transaksi INNER JOIN paket ON paket.id_paket=detailtransaksi.id_paket INNER JOIN outlet ON outlet.id_outlet=transaksi.id_outlet ORDER BY transaksi.id_transaksi DESC LIMIT 10`;
...
app.post('/laporan/general', (req, res) => {
    ...
    let sql = `SELECT transaksi.*,member.nama as nama_member , detailtransaksi.*, paket.*, outlet.*, (SELECT SUM(total_harga) FROM detailtransaksi INNER JOIN transaksi ON transaksi.id_transaksi = detailtransaksi.id_transaksi and tgl between '${tgl_awal}' and '${tgl_akhir}') as total FROM transaksi INNER JOIN member ON member.id_member = transaksi.id_member INNER JOIN detailtransaksi ON detailtransaksi.id_transaksi = transaksi.id_transaksi INNER JOIN paket ON paket.id_paket=detailtransaksi.id_paket INNER JOIN outlet ON outlet.id_outlet=transaksi.id_outlet and tgl between '${tgl_awal}' and '${tgl_akhir}'`;
    let data = db.query(sql, (err, result) => {
        if(err) throw err
        res.json(result)
    })
...
app.post('/laporan', (req, res) => {
    ...
    let sql = `SELECT transaksi.*,member.nama as nama_member , detailtransaksi.*, paket.*, outlet.*, (SELECT SUM(total_harga) FROM detailtransaksi INNER JOIN transaksi ON transaksi.id_transaksi = detailtransaksi.id_transaksi and dibayar = "dibayar" and tgl between '${tgl_awal}' and '${tgl_akhir}') as total FROM transaksi INNER JOIN member ON member.id_member = transaksi.id_member INNER JOIN detailtransaksi ON detailtransaksi.id_transaksi = transaksi.id_transaksi INNER JOIN paket ON paket.id_paket=detailtransaksi.id_paket INNER JOIN outlet ON outlet.id_outlet = transaksi.id_outlet and tgl between '${tgl_awal}' and '${tgl_akhir}'`;
    let data = db.query(sql, (err, result) => {
        if(err) throw err
        res.json(result)
    })
    // let tgl_awal = {
    //     tgl: req.body.tgl_awal
    // }
    // let tgl_akhir = {
    //     tgl: req.body.tgl_akhir
    // }
    // let id_outlet = req.params.id_outlet
    // let sql = `SELECT transaksi.*,member.nama as nama_member , detailtransaksi.*, paket.*, outlet.*, (SELECT SUM(total_harga) FROM detailtransaksi INNER JOIN transaksi ON transaksi.id_transaksi = detailtransaksi.id_transaksi and dibayar = "dibayar" and tgl between '${tgl_awal}' and '${tgl_akhir}') as total FROM transaksi INNER JOIN member ON member.id_member = transaksi.id_member INNER JOIN detailtransaksi ON detailtransaksi.id_transaksi = transaksi.id_transaksi INNER JOIN paket ON paket.id_paket=detailtransaksi.id_paket INNER JOIN outlet ON outlet.id_outlet = transaksi.id_outlet and tgl between '${tgl_awal}' and '${tgl_akhir}'`;
    // let data = db.query(sql, (err, result) => {
    ...
    // };
    // transaksi.findAll(
    //     {where : {"fieldOfYourDate" : {[Op.between] : [tgl_awal , tgl_akhir ]}}}
    // )
    //     .then(result => {
    //         res.json(result)
    //     })
...
app.get("/:id_transaksi", (req,res) => {
    detailtransaksi.findOne({ where: {id_transaksi: req.params.id_transaksi},
        include: [
            {
                model: transaksi,
                include: [
...
app.put("/bayar/:id_transaksi", (req,res) => {
    ...
    // let total_hargaa;
    let tagihan = detailtransaksi.findOne({ where: { id_transaksi: param } })
    tagihan.then(function(result) { //Dimasukin result semua agar data variabelnya bisa dipakai (karena lokal)
        ...
        if (total_hargaa > req.body.total_bayar) {
            ...
            detailtransaksi.update(data, { where: { id_transaksi: param } })
                .then(() => {
                    transaksi.update(data2, {where: {id_transaksi : param}})
                    .then(() => {
                        transaksi.update(data3, {where: {id_transaksi : param}})
                        .then(() => {
                            detailtransaksi.update(data4, {where: {id_transaksi : param}})
                            .then(() => {
                            res.json({
...
//endpoint menghapus data transaksi dan detailtransaksi, METHOD: DELETE, function: destroy
